...
**v2 Pipeline (Option A, Session 37):** Phase 1 (Railway, 5am): ALL candidates that pass triage get a research brief stored in Supabase KB with status `ready`, including full Jina markdown source text. Phase 2 (Remote Trigger, 5:27am, Claude Opus, $0): fetches hydrated briefs via `GET /api/v2/briefs-for-processing/:id`, Claude writes/evaluates/checks fabrication itself, posts results via `POST /api/v2/store-produced`. Brief lifecycle: `ready` → `processing` → `produced`/`held`/`duplicate`/`development` → `approved`/`rejected`. Score ≥ 75 + CLEAN fabrication = `produced` (inbox). Score < 75 or SUSPECT = `held` (retryable). Every action logged to `editorial_decisions` in Supabase.
