...
**v2 Content Pipeline (Sessions 14-37, Option A architecture):** Two-phase split. **Phase 1 (Railway, 5am):** Discovery + triage + Research Agent → briefs stored in Supabase KB with full Jina markdown. **Phase 2 (Remote Trigger, 5:27am, Claude Opus, Max tokens = $0):** Claude itself writes consulting-quality entries, evaluates against McKinsey 6-check test, checks fabrication, computes score — no `new Anthropic()` API calls. Fetches briefs via `GET /api/v2/briefs-for-processing`, posts results via `POST /api/v2/store-produced`. Brief lifecycle: ready → processing → produced/held/duplicate/development → approved/rejected. Every action logged to `editorial_decisions` in Supabase. **Session 37:** Option A built — tsconfig fix (Railway build failure), 2 new API routes, Remote Trigger upgraded to Opus with full writing/eval/fabrication prompt.

**Knowledge Base (Sessions 22-28):** Supabase (PostgreSQL + pgvector). 265 sources embedded (Jina v3, 512 dims). All 5 KB phases complete. 10 platform engineering principles enforced in code. Prompts versioned in `intake-server/prompts/`. Phase 1 stores all source markdown in KB; Phase 2 reads it via hydrated briefs.
...
**v2 Pipeline (Sessions 37-38):** Phase 1 (Railway, 5am): ALL candidates that pass triage get a research brief stored in Supabase KB with status `ready`, including full Jina markdown source text. Phase 2 (Remote Trigger, 5:27am, Claude Opus, $0): reads briefs and sources directly from Supabase REST API. **Mandatory evaluation loop:** Write v1 → Evaluate (McKinsey 6-check) → ALWAYS refine to v2 → Re-evaluate → Fabrication check → Score. Requires Custom network access environment with `*.supabase.co` in allowlist, credentials via env vars ($SB_URL, $SB_KEY). Brief lifecycle: `ready` → `produced`/`held` → `approved`/`rejected`. Score ≥ 75 + CLEAN = `produced`. Every action logged to `editorial_decisions`. `iteration_logs` table and `v2_iterations` column exist but remote agent currently skips logging steps (non-blocking — eval loop runs correctly). **Known gaps (session 38 audit):** GAP6 (trigger-batch burns API credits), GAP7 (no TL writer mode), GAP8 (v2 scoring differs from scorer.js), iteration logging not stored by remote agent.
